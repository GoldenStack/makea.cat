...
fn draw_cat(dt: &mut DrawTarget, base: &Transform) {
    ...
    dt.set_transform(&Transform::rotation(Angle::degrees(20.)).then_translate(Vector::new(-45., 21.)).then(&base));
    dt.stroke(&legs, &black, &stroke, &DrawOptions::new());
    dt.fill(&legs, &random_color(), &DrawOptions::new());

    dt.set_transform(&Transform::rotation(Angle::degrees(5.)).then_translate(Vector::new(-25., 26.)).then(&base));
    dt.stroke(&legs, &black, &stroke, &DrawOptions::new());
    dt.fill(&legs, &random_color(), &DrawOptions::new());
